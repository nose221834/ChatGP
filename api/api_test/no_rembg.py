...
async def generate_car_by_chatgpt(input_text_model:InputTextModel = Depends(),api_key: str = Security(validate_api_key)):
    ...
    text_inputted_by_user = translation(input_text_model.text_inputted_by_user,'JA','EN-US')
    
    # ユーザー入力が入力トークン数の上限(5トークン)を上回っていないか検証
    # 問題がない場合,ChatGPTで車の画像とステータスを生成
    if validate_token_count(text_inputted_by_user,5):
        url_car_img, [player_luck,car_name,text_car_status] = await asyncio.gather(
            generate_car_img_by_chatgpt_no_rembg(text_inputted_by_user,IMAGE_MODEL_CHATGPT,img_size),
            generate_car_status_by_chatgpt(text_inputted_by_user)
        )

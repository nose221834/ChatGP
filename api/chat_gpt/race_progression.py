...
def create_prompt_generating_race_scenario(race_moderator_model:RaceModeratorModel):
    """
...
def generate_race_scenario_by_chatgpt(race_moderator_model:RaceModeratorModel):
    """
        ChatGPTがユーザーの入力を受け取り,その入力を元にシナリオを作成する

        Args:
            race_moderator_model (RaceModeratorModel):レースに参加している車の情報
        Returns:  
    ...
    while(not(validate_chat_gpt_output_count(text_split,item_count_in_format,number_of_generation))): 
        prompt_system,prompt_user = create_prompt_generating_race_scenario(race_moderator_model)

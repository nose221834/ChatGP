...
def get_data(db:str,table:str,key:str,id:int) -> list:
    ...
    #　table内から後から与えられる"?"と一意するデータをkey(カラム)の中から検索し,全カラムのデータを所得する
    query = f"SELECT * FROM {table} WHERE {key} = ?"

    # queryの"?"にidを代入し,クエリを実行
    c.execute(query, (id,))

    #クエリの結果を全て取得しリストに格納 形式:[(カラム1-1,カラム2-1,カラム3-1,・・・),(カラム2-1,カラム2-2,カラム3-2,・・・),・・・]
